 ...
 *
 * $Log$
 * Revision 1.1  1996/03/01 11:39:30  mclareni
 * Initial revision
 *
 */
...
char *admin;              /* mailing address of the stagerd admin */
...
read_config_file()
{
  ...
  if (syslogchn)
#ifdef LOG_AUTH
    openlog("stagerd",LOG_PID | LOG_CONS,c_syslogf);
#else
    openlog("stagerd",LOG_PID | LOG_CONS);
#endif
 ...
 * Master:
 *
 * call:    stagerd [-p] -m mnum configfile
 *
 ...
 *
 * example: stagerd -m 0 /usr/local/etc/stagerd.conf
 *
 * Slave:
 *
 * call:    stagerd [-p] -s configfile
 *
 *          where configfile is the full path to the configfile
 *          -p as explained above
 *
 * example: stagerd -s /usr/local/etc/stagerd.conf
 *
 *************************************************************************/
...
main(int argc, char **argv)
{
  ...
  /* this umask is for the internal use by the stager -
